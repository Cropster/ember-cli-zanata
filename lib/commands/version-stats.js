  ...
  start(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._prepareFolder(options).then(() => {
        return this._loadSources(options);
      }).then((sources) => {
        return Promise.all(sources.map((source) => {
          return this._loadDocumentStats(projectId, version, source.name, options);
        }));
      }).then(resolve).catch(reject).finally(() => this._cleanupTmpFolder(options.tmpDir));
    });
  },
  ...
  _loadSources(options) {
    ...
    return new Promise((resolve, reject) => {
      this._getProject(options)
        ...
        .on('end_pull', (data) => {
          resolve(data)
        }).pullSources(projectId, version, { potdir: options.tmpDir });
    });
  },

  /**
   * Prepare the tmp folder.
   * The sources are cached there.
   *
